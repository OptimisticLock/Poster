...
_.extend(Connection.prototype, {                                                                                     // 468
  ...
  /**                                                                                                                // 506
   * @memberOf Meteor                                                                                                // 507
   * @summary Subscribe to a record set.  Returns a handle that provides                                             // 508
   * `stop()` and `ready()` methods.                                                                                 // 509
   * @locus Client                                                                                                   // 510
   * @param {String} name Name of the subscription.  Matches the name of the                                         // 511
   * server's `publish()` call.                                                                                      // 512
   * @param {EJSONable} [arg1,arg2...] Optional arguments passed to publisher                                        // 513
   * function on server.                                                                                             // 514
   * @param {Function|Object} [callbacks] Optional. May include `onStop`                                             // 515
   * and `onReady` callbacks. If there is an error, it is passed as an                                               // 516
   * argument to `onStop`. If a function is passed instead of an object, it                                          // 517
   * is interpreted as an `onReady` callback.                                                                        // 518
   */                                                                                                                // 519
  subscribe: function (name /* .. [arguments] .. (callback|callbacks) */) {                                          // 520
    var self = this;                                                                                                 // 521
                                                                                                                     // 522
    var params = Array.prototype.slice.call(arguments, 1);                                                           // 523
    var callbacks = {};                                                                                              // 524
    if (params.length) {                                                                                             // 525
      var lastParam = params[params.length - 1];                                                                     // 526
      if (_.isFunction(lastParam)) {                                                                                 // 527
        callbacks.onReady = params.pop();                                                                            // 528
      } else if (lastParam &&                                                                                        // 529
        // XXX COMPAT WITH 1.0.3.1 onError used to exist, but now we use                                             // 530
        // onStop with an error callback instead.                                                                    // 531
        _.any([lastParam.onReady, lastParam.onError, lastParam.onStop],                                              // 532
          _.isFunction)) {                                                                                           // 533
        callbacks = params.pop();                                                                                    // 534
      }                                                                                                              // 535
    }                                                                                                                // 536
                                                                                                                     // 537
    // Is there an existing sub with the same name and param, run in an                                              // 538
    // invalidated Computation? This will happen if we are rerunning an                                              // 539
    // existing computation.                                                                                         // 540
    //                                                                                                               // 541
    // For example, consider a rerun of:                                                                             // 542
    //                                                                                                               // 543
    //     Tracker.autorun(function () {                                                                             // 544
    //       Meteor.subscribe("foo", Session.get("foo"));                                                            // 545
    //       Meteor.subscribe("bar", Session.get("bar"));                                                            // 546
    //     });                                                                                                       // 547
    //                                                                                                               // 548
    // If "foo" has changed but "bar" has not, we will match the "bar"                                               // 549
    // subcribe to an existing inactive subscription in order to not                                                 // 550
    // unsub and resub the subscription unnecessarily.                                                               // 551
    //                                                                                                               // 552
    // We only look for one such sub; if there are N apparently-identical subs                                       // 553
    // being invalidated, we will require N matching subscribe calls to keep                                         // 554
    // them all active.                                                                                              // 555
    var existing = _.find(self._subscriptions, function (sub) {                                                      // 556
      return sub.inactive && sub.name === name &&                                                                    // 557
        EJSON.equals(sub.params, params);                                                                            // 558
    });                                                                                                              // 559
                                                                                                                     // 560
    var id;                                                                                                          // 561
    if (existing) {                                                                                                  // 562
      id = existing.id;                                                                                              // 563
      existing.inactive = false; // reactivate                                                                       // 564
                                                                                                                     // 565
      if (callbacks.onReady) {                                                                                       // 566
        // If the sub is not already ready, replace any ready callback with the                                      // 567
        // one provided now. (It's not really clear what users would expect for                                      // 568
        // an onReady callback inside an autorun; the semantics we provide is                                        // 569
        // that at the time the sub first becomes ready, we call the last                                            // 570
        // onReady callback provided, if any.)                                                                       // 571
        if (!existing.ready)                                                                                         // 572
          existing.readyCallback = callbacks.onReady;                                                                // 573
      }                                                                                                              // 574
                                                                                                                     // 575
      // XXX COMPAT WITH 1.0.3.1 we used to have onError but now we call                                             // 576
      // onStop with an optional error argument                                                                      // 577
      if (callbacks.onError) {                                                                                       // 578
        // Replace existing callback if any, so that errors aren't                                                   // 579
        // double-reported.                                                                                          // 580
        existing.errorCallback = callbacks.onError;                                                                  // 581
      }                                                                                                              // 582
                                                                                                                     // 583
      if (callbacks.onStop) {                                                                                        // 584
        existing.stopCallback = callbacks.onStop;                                                                    // 585
      }                                                                                                              // 586
    } else {                                                                                                         // 587
      // New sub! Generate an id, save it locally, and send message.                                                 // 588
      id = Random.id();                                                                                              // 589
      self._subscriptions[id] = {                                                                                    // 590
        id: id,                                                                                                      // 591
        name: name,                                                                                                  // 592
        params: EJSON.clone(params),                                                                                 // 593
        inactive: false,                                                                                             // 594
        ready: false,                                                                                                // 595
        readyDeps: new Tracker.Dependency,                                                                           // 596
        readyCallback: callbacks.onReady,                                                                            // 597
        // XXX COMPAT WITH 1.0.3.1 #errorCallback                                                                    // 598
        errorCallback: callbacks.onError,                                                                            // 599
        stopCallback: callbacks.onStop,                                                                              // 600
        connection: self,                                                                                            // 601
        remove: function() {                                                                                         // 602
          delete this.connection._subscriptions[this.id];                                                            // 603
          this.ready && this.readyDeps.changed();                                                                    // 604
        },                                                                                                           // 605
        stop: function() {                                                                                           // 606
          this.connection._send({msg: 'unsub', id: id});                                                             // 607
          this.remove();                                                                                             // 608
                                                                                                                     // 609
          if (callbacks.onStop) {                                                                                    // 610
            callbacks.onStop();                                                                                      // 611
          }                                                                                                          // 612
        }                                                                                                            // 613
      };                                                                                                             // 614
      self._send({msg: 'sub', id: id, name: name, params: params});                                                  // 615
    }                                                                                                                // 616
                                                                                                                     // 617
    // return a handle to the application.                                                                           // 618
    var handle = {                                                                                                   // 619
      stop: function () {                                                                                            // 620
        if (!_.has(self._subscriptions, id))                                                                         // 621
          return;                                                                                                    // 622
                                                                                                                     // 623
        self._subscriptions[id].stop();                                                                              // 624
      },                                                                                                             // 625
      ready: function () {                                                                                           // 626
        // return false if we've unsubscribed.                                                                       // 627
        if (!_.has(self._subscriptions, id))                                                                         // 628
          return false;                                                                                              // 629
        var record = self._subscriptions[id];                                                                        // 630
        record.readyDeps.depend();                                                                                   // 631
        return record.ready;                                                                                         // 632
      },                                                                                                             // 633
      subscriptionId: id                                                                                             // 634
    };                                                                                                               // 635
                                                                                                                     // 636
    if (Tracker.active) {                                                                                            // 637
      // We're in a reactive computation, so we'd like to unsubscribe when the                                       // 638
      // computation is invalidated... but not if the rerun just re-subscribes                                       // 639
      // to the same subscription!  When a rerun happens, we use onInvalidate                                        // 640
      // as a change to mark the subscription "inactive" so that it can                                              // 641
      // be reused from the rerun.  If it isn't reused, it's killed from                                             // 642
      // an afterFlush.                                                                                              // 643
      Tracker.onInvalidate(function (c) {                                                                            // 644
        if (_.has(self._subscriptions, id))                                                                          // 645
          self._subscriptions[id].inactive = true;                                                                   // 646
                                                                                                                     // 647
        Tracker.afterFlush(function () {                                                                             // 648
          if (_.has(self._subscriptions, id) &&                                                                      // 649
              self._subscriptions[id].inactive)                                                                      // 650
            handle.stop();                                                                                           // 651
        });                                                                                                          // 652
      });                                                                                                            // 653
    }                                                                                                                // 654
    ...
    return handle;                                                                                                   // 656
                                                                                                                     ...
                                                                                                                     // 658
  // options:                                                                                                        // 659
  // - onLateError {Function(error)} called if an error was received after the ready event.                          // 660
  //     (errors received before ready cause an error to be thrown)                                                  // 661
  _subscribeAndWait: function (name, args, options) {                                                                // 662
    var self = this;                                                                                                 // 663
    var f = new Future();                                                                                            // 664
    var ready = false;                                                                                               // 665
    var handle;                                                                                                      // 666
    args = args || [];                                                                                               // 667
    args.push({                                                                                                      // 668
      onReady: function () {                                                                                         // 669
        ready = true;                                                                                                // 670
        f['return']();                                                                                               // 671
      },                                                                                                             // 672
      onError: function (e) {                                                                                        // 673
        if (!ready)                                                                                                  // 674
          f['throw'](e);                                                                                             // 675
        else                                                                                                         // 676
          options && options.onLateError && options.onLateError(e);                                                  // 677
      }                                                                                                              // 678
    });                                                                                                              // 679
                                                                                                                     // 680
    handle = self.subscribe.apply(self, [name].concat(args));                                                        // 681
    f.wait();                                                                                                        // 682
    return handle;                                                                                                   // 683
  },                                                                                                                 // 684
                                                                                                                     // 685
  methods: function (methods) {                                                                                      // 686
    var self = this;                                                                                                 // 687
    _.each(methods, function (func, name) {                                                                          // 688
      if (typeof func !== 'function')                                                                                // 689
        throw new Error("Method '" + name + "' must be a function");                                                 // 690
      if (self._methodHandlers[name])                                                                                // 691
        throw new Error("A method named '" + name + "' is already defined");                                         // 692
      self._methodHandlers[name] = func;                                                                             // 693
    });                                                                                                              // 694
  },                                                                                                                 // 695
                                                                                                                     // 696
  /**                                                                                                                // 697
   * @memberOf Meteor                                                                                                // 698
   * @summary Invokes a method passing any number of arguments.                                                      // 699
   * @locus Anywhere                                                                                                 // 700
   * @param {String} name Name of method to invoke                                                                   // 701
   * @param {EJSONable} [arg1,arg2...] Optional method arguments                                                     // 702
   * @param {Function} [asyncCallback] Optional callback, which is called asynchronously with the error or result after the method is complete. If not provided, the method runs synchronously if possible (see below).
   */                                                                                                                // 704
  call: function (name /* .. [arguments] .. callback */) {                                                           // 705
    // if it's a function, the last argument is the result callback,                                                 // 706
    // not a parameter to the remote method.                                                                         // 707
    var args = Array.prototype.slice.call(arguments, 1);                                                             // 708
    if (args.length && typeof args[args.length - 1] === "function")                                                  // 709
      var callback = args.pop();                                                                                     // 710
    return this.apply(name, args, callback);                                                                         // 711
  },                                                                                                                 // 712
                                                                                                                     // 713
  // @param options {Optional Object}                                                                                // 714
  //   wait: Boolean - Should we wait to call this until all current methods                                         // 715
  //                   are fully finished, and block subsequent method calls                                         // 716
  //                   until this method is fully finished?                                                          // 717
  //                   (does not affect methods called from within this method)                                      // 718
  //   onResultReceived: Function - a callback to call as soon as the method                                         // 719
  //                                result is received. the data written by                                          // 720
  //                                the method may not yet be in the cache!                                          // 721
  //   returnStubValue: Boolean - If true then in cases where we would have                                          // 722
  //                              otherwise discarded the stub's return value                                        // 723
  //                              and returned undefined, instead we go ahead                                        // 724
  //                              and return it.  Specifically, this is any                                          // 725
  //                              time other than when (a) we are already                                            // 726
  //                              inside a stub or (b) we are in Node and no                                         // 727
  //                              callback was provided.  Currently we require                                       // 728
  //                              this flag to be explicitly passed to reduce                                        // 729
  //                              the likelihood that stub return values will                                        // 730
  //                              be confused with server return values; we                                          // 731
  //                              may improve this in future.                                                        // 732
  // @param callback {Optional Function}                                                                             // 733
                                                                                                                     // 734
  /**                                                                                                                // 735
   * @memberOf Meteor                                                                                                // 736
   * @summary Invoke a method passing an array of arguments.                                                         // 737
   * @locus Anywhere                                                                                                 // 738
   * @param {String} name Name of method to invoke                                                                   // 739
   * @param {EJSONable[]} args Method arguments                                                                      // 740
   * @param {Object} [options]                                                                                       // 741
   * @param {Boolean} options.wait (Client only) If true, don't send this method until all previous method calls have completed, and don't send any subsequent method calls until this one is completed.
   * @param {Function} options.onResultReceived (Client only) This callback is invoked with the error or result of the method (just like `asyncCallback`) as soon as the error or result is available. The local cache may not yet reflect the writes performed by the method.
   * @param (Boolean) options.noRetry (Client only) if true, don't send this method again on reload, simply call the callback an error with the error code 'invocation-failed'.
   * @param {Function} [asyncCallback] Optional callback; same semantics as in [`Meteor.call`](#meteor_call).        // 745
   */                                                                                                                // 746
  apply: function (name, args, options, callback) {                                                                  // 747
    var self = this;                                                                                                 // 748
                                                                                                                     // 749
    // We were passed 3 arguments. They may be either (name, args, options)                                          // 750
    // or (name, args, callback)                                                                                     // 751
    if (!callback && typeof options === 'function') {                                                                // 752
    ...
    options = options || {};                                                                                         // 756
                                                                                                                     // 757
    if (callback) {                                                                                                  // 758
      // XXX would it be better form to do the binding in stream.on,                                                 // 759
      // or caller, instead of here?                                                                                 // 760
      // XXX improve error message (and how we report it)                                                            // 761
      callback = Meteor.bindEnvironment(                                                                             // 762
        callback,                                                                                                    // 763
        "delivering result of invoking '" + name + "'"                                                               // 764
      );                                                                                                             // 765
    }                                                                                                                // 766
                                                                                                                     // 767
    // Keep our args safe from mutation (eg if we don't send the message for a                                       // 768
    // while because of a wait method).                                                                              // 769
    args = EJSON.clone(args);                                                                                        // 770
                                                                                                                     // 771
    // Lazily allocate method ID once we know that it'll be needed.                                                  // 772
    var methodId = (function () {                                                                                    // 773
      var id;                                                                                                        // 774
      return function () {                                                                                           // 775
        if (id === undefined)                                                                                        // 776
          id = '' + (self._nextMethodId++);                                                                          // 777
        return id;                                                                                                   // 778
      };                                                                                                             // 779
    })();                                                                                                            // 780
                                                                                                                     // 781
    var enclosing = DDP._CurrentInvocation.get();                                                                    // 782
    var alreadyInSimulation = enclosing && enclosing.isSimulation;                                                   // 783
                                                                                                                     // 784
    // Lazily generate a randomSeed, only if it is requested by the stub.                                            // 785
    // The random streams only have utility if they're used on both the client                                       // 786
    // and the server; if the client doesn't generate any 'random' values                                            // 787
    // then we don't expect the server to generate any either.                                                       // 788
    // Less commonly, the server may perform different actions from the client,                                      // 789
    // and may in fact generate values where the client did not, but we don't                                        // 790
    // have any client-side values to match, so even here we may as well just                                        // 791
    // use a random seed on the server.  In that case, we don't pass the                                             // 792
    // randomSeed to save bandwidth, and we don't even generate it to save a                                         // 793
    // bit of CPU and to avoid consuming entropy.                                                                    // 794
    var randomSeed = null;                                                                                           // 795
    var randomSeedGenerator = function () {                                                                          // 796
      if (randomSeed === null) {                                                                                     // 797
        randomSeed = DDPCommon.makeRpcSeed(enclosing, name);                                                         // 798
      }                                                                                                              // 799
      return randomSeed;                                                                                             // 800
    };                                                                                                               // 801
                                                                                                                     // 802
    // Run the stub, if we have one. The stub is supposed to make some                                               // 803
    // temporary writes to the database to give the user a smooth experience                                         // 804
    // until the actual result of executing the method comes back from the                                           // 805
    // server (whereupon the temporary writes to the database will be reversed                                       // 806
    // during the beginUpdate/endUpdate process.)                                                                    // 807
    //                                                                                                               // 808
    // Normally, we ignore the return value of the stub (even if it is an                                            // 809
    // exception), in favor of the real return value from the server. The                                            // 810
    // exception is if the *caller* is a stub. In that case, we're not going                                         // 811
    // to do a RPC, so we use the return value of the stub as our return                                             // 812
    // value.                                                                                                        // 813
                                                                                                                     // 814
    var stub = self._methodHandlers[name];                                                                           // 815
    if (stub) {                                                                                                      // 816
      var setUserId = function(userId) {                                                                             // 817
        self.setUserId(userId);                                                                                      // 818
      };                                                                                                             // 819
                                                                                                                     // 820
      var invocation = new DDPCommon.MethodInvocation({                                                              // 821
        isSimulation: true,                                                                                          // 822
        userId: self.userId(),                                                                                       // 823
        setUserId: setUserId,                                                                                        // 824
        randomSeed: function () { return randomSeedGenerator(); }                                                    // 825
      });                                                                                                            // 826
                                                                                                                     // 827
      if (!alreadyInSimulation)                                                                                      // 828
        self._saveOriginals();                                                                                       // 829
                                                                                                                     // 830
      try {                                                                                                          // 831
        // Note that unlike in the corresponding server code, we never audit                                         // 832
        // that stubs check() their arguments.                                                                       // 833
        var stubReturnValue = DDP._CurrentInvocation.withValue(invocation, function () {                             // 834
          if (Meteor.isServer) {                                                                                     // 835
            // Because saveOriginals and retrieveOriginals aren't reentrant,                                         // 836
            // don't allow stubs to yield.                                                                           // 837
            return Meteor._noYieldsAllowed(function () {                                                             // 838
              // re-clone, so that the stub can't affect our caller's values                                         // 839
              return stub.apply(invocation, EJSON.clone(args));                                                      // 840
            });                                                                                                      // 841
          } else {                                                                                                   // 842
        ...
        var exception = e;                                                                                           // 848
                                                                                                                     ...
                                                                                                                     // 850
      if (!alreadyInSimulation)                                                                                      // 851
        self._retrieveAndStoreOriginals(methodId());                                                                 // 852
    }                                                                                                                // 853
                                                                                                                     // 854
    // If we're in a simulation, stop and return the result we have,                                                 // 855
    // rather than going on to do an RPC. If there was no stub,                                                      // 856
    // we'll end up returning undefined.                                                                             // 857
    if (alreadyInSimulation) {                                                                                       // 858
      if (callback) {                                                                                                // 859
        callback(exception, stubReturnValue);                                                                        // 860
        return undefined;                                                                                            // 861
      }                                                                                                              // 862
      if (exception)                                                                                                 // 863
        throw exception;                                                                                             // 864
      return stubReturnValue;                                                                                        // 865
    }                                                                                                                // 866
                                                                                                                     // 867
    // If an exception occurred in a stub, and we're ignoring it                                                     // 868
    // because we're doing an RPC and want to use what the server                                                    // 869
    // returns instead, log it so the developer knows                                                                // 870
    // (unless they explicitly ask to see the error).                                                                // 871
    //                                                                                                               // 872
    // Tests can set the 'expected' flag on an exception so it won't                                                 // 873
    // go to log.                                                                                                    // 874
    if (exception) {                                                                                                 // 875
      if (options.throwStubExceptions) {                                                                             // 876
        throw exception;                                                                                             // 877
      } else if (!exception.expected) {                                                                              // 878
        Meteor._debug("Exception while simulating the effect of invoking '" +                                        // 879
          name + "'", exception, exception.stack);                                                                   // 880
      }                                                                                                              // 881
    }                                                                                                                // 882
                                                                                                                     // 883
                                                                                                                     // 884
    // At this point we're definitely doing an RPC, and we're going to                                               // 885
    // return the value of the RPC to the caller.                                                                    // 886
                                                                                                                     // 887
    // If the caller didn't give a callback, decide what to do.                                                      // 888
    if (!callback) {                                                                                                 // 889
      if (Meteor.isClient) {                                                                                         // 890
        // On the client, we don't have fibers, so we can't block. The                                               // 891
        // only thing we can do is to return undefined and discard the                                               // 892
        // result of the RPC. If an error occurred then print the error                                              // 893
        // to the console.                                                                                           // 894
        callback = function (err) {                                                                                  // 895
          err && Meteor._debug("Error invoking Method '" + name + "':",                                              // 896
                               err.message);                                                                         // 897
        };                                                                                                           // 898
      } else {                                                                                                       // 899
        // On the server, make the function synchronous. Throw on                                                    // 900
        // errors, return on success.                                                                                // 901
        var future = new Future;                                                                                     // 902
        callback = future.resolver();                                                                                // 903
      }                                                                                                              // 904
    }                                                                                                                // 905
    // Send the RPC. Note that on the client, it is important that the                                               // 906
    // stub have finished before we send the RPC, so that we know we have                                            // 907
    // a complete list of which local documents the stub wrote.                                                      // 908
    var message = {                                                                                                  // 909
      msg: 'method',                                                                                                 // 910
      method: name,                                                                                                  // 911
      params: args,                                                                                                  // 912
      id: methodId()                                                                                                 // 913
                                                                                                                     ...
                                                                                                                     // 915
    // Send the randomSeed only if we used it                                                                        // 916
    if (randomSeed !== null) {                                                                                       // 917
      message.randomSeed = randomSeed;                                                                               // 918
    }                                                                                                                // 919
                                                                                                                     // 920
    var methodInvoker = new MethodInvoker({                                                                          // 921
      methodId: methodId(),                                                                                          // 922
      callback: callback,                                                                                            // 923
      connection: self,                                                                                              // 924
      onResultReceived: options.onResultReceived,                                                                    // 925
      wait: !!options.wait,                                                                                          // 926
      message: message,                                                                                              // 927
      noRetry: !!options.noRetry                                                                                     // 928
    });                                                                                                              // 929
                                                                                                                     // 930
    if (options.wait) {                                                                                              // 931
      // It's a wait method! Wait methods go in their own block.                                                     // 932
      self._outstandingMethodBlocks.push(                                                                            // 933
        {wait: true, methods: [methodInvoker]});                                                                     // 934
    } else {                                                                                                         // 935
      // Not a wait method. Start a new block if the previous block was a wait                                       // 936
      // block, and add it to the last block of methods.                                                             // 937
      if (_.isEmpty(self._outstandingMethodBlocks) ||                                                                // 938
          _.last(self._outstandingMethodBlocks).wait)                                                                // 939
        self._outstandingMethodBlocks.push({wait: false, methods: []});                                              // 940
      _.last(self._outstandingMethodBlocks).methods.push(methodInvoker);                                             // 941
    }                                                                                                                // 942
                                                                                                                     // 943
    // If we added it to the first block, send it out now.                                                           // 944
    if (self._outstandingMethodBlocks.length === 1)                                                                  // 945
      methodInvoker.sendMessage();                                                                                   // 946
                                                                                                                     // 947
    // If we're using the default callback on the server,                                                            // 948
    // block waiting for the result.                                                                                 // 949
    if (future) {                                                                                                    // 950
      return future.wait();                                                                                          // 951
    }                                                                                                                // 952
    return options.returnStubValue ? stubReturnValue : undefined;                                                    // 953
  },                                                                                                                 // 954
                                                                                                                     // 955
  // Before calling a method stub, prepare all stores to track changes and allow                                     // 956
  // _retrieveAndStoreOriginals to get the original versions of changed                                              // 957
  // documents.                                                                                                      // 958
  _saveOriginals: function () {                                                                                      // 959
    var self = this;                                                                                                 // 960
    _.each(self._stores, function (s) {                                                                              // 961
      s.saveOriginals();                                                                                             // 962
    });                                                                                                              // 963
  },                                                                                                                 // 964
  // Retrieves the original versions of all documents modified by the stub for                                       // 965
  // method 'methodId' from all stores and saves them to _serverDocuments (keyed                                     // 966
  // by document) and _documentsWrittenByStub (keyed by method ID).                                                  // 967
  _retrieveAndStoreOriginals: function (methodId) {                                                                  // 968
    var self = this;                                                                                                 // 969
    if (self._documentsWrittenByStub[methodId])                                                                      // 970
      throw new Error("Duplicate methodId in _retrieveAndStoreOriginals");                                           // 971
                                                                                                                     // 972
    var docsWritten = [];                                                                                            // 973
    _.each(self._stores, function (s, collection) {                                                                  // 974
      var originals = s.retrieveOriginals();                                                                         // 975
      // not all stores define retrieveOriginals                                                                     // 976
      if (!originals)                                                                                                // 977
        return;                                                                                                      // 978
      originals.forEach(function (doc, id) {                                                                         // 979
        docsWritten.push({collection: collection, id: id});                                                          // 980
        if (!_.has(self._serverDocuments, collection))                                                               // 981
          self._serverDocuments[collection] = new MongoIDMap;                                                        // 982
        var serverDoc = self._serverDocuments[collection].setDefault(id, {});                                        // 983
        if (serverDoc.writtenByStubs) {                                                                              // 984
          // We're not the first stub to write this doc. Just add our method ID                                      // 985
          // to the record.                                                                                          // 986
          serverDoc.writtenByStubs[methodId] = true;                                                                 // 987
        } else {                                                                                                     // 988
          // First stub! Save the original value and our method ID.                                                  // 989
          serverDoc.document = doc;                                                                                  // 990
          serverDoc.flushCallbacks = [];                                                                             // 991
          serverDoc.writtenByStubs = {};                                                                             // 992
          serverDoc.writtenByStubs[methodId] = true;                                                                 // 993
        }                                                                                                            // 994
      });                                                                                                            // 995
    });                                                                                                              // 996
    if (!_.isEmpty(docsWritten)) {                                                                                   // 997
      self._documentsWrittenByStub[methodId] = docsWritten;                                                          // 998
    }                                                                                                                // 999
  },                                                                                                                 // 1000
                                                                                                                     // 1001
  // This is very much a private function we use to make the tests                                                   // 1002
  // take up fewer server resources after they complete.                                                             // 1003
  _unsubscribeAll: function () {                                                                                     // 1004
    var self = this;                                                                                                 // 1005
    _.each(_.clone(self._subscriptions), function (sub, id) {                                                        // 1006
      // Avoid killing the autoupdate subscription so that developers                                                // 1007
      // still get hot code pushes when writing tests.                                                               // 1008
      //                                                                                                             // 1009
      // XXX it's a hack to encode knowledge about autoupdate here,                                                  // 1010
      // but it doesn't seem worth it yet to have a special API for                                                  // 1011
      // subscriptions to preserve after unit tests.                                                                 // 1012
      if (sub.name !== 'meteor_autoupdate_clientVersions') {                                                         // 1013
        self._subscriptions[id].stop();                                                                              // 1014
      }                                                                                                              // 1015
    });                                                                                                              // 1016
  },                                                                                                                 // 1017
                                                                                                                     // 1018
  // Sends the DDP stringification of the given message object                                                       // 1019
  _send: function (obj) {                                                                                            // 1020
    var self = this;                                                                                                 // 1021
    self._stream.send(DDPCommon.stringifyDDP(obj));                                                                  // 1022
  },                                                                                                                 // 1023
                                                                                                                     // 1024
  // We detected via DDP-level heartbeats that we've lost the                                                        // 1025
  // connection.  Unlike `disconnect` or `close`, a lost connection                                                  // 1026
  // will be automatically retried.                                                                                  // 1027
  _lostConnection: function (error) {                                                                                // 1028
    var self = this;                                                                                                 // 1029
    self._stream._lostConnection(error);                                                                             // 1030
  },                                                                                                                 // 1031
                                                                                                                     // 1032
  /**                                                                                                                // 1033
   * @summary Get the current connection status. A reactive data source.                                             // 1034
   * @locus Client                                                                                                   // 1035
   * @memberOf Meteor                                                                                                // 1036
   */                                                                                                                // 1037
  status: function (/*passthrough args*/) {                                                                          // 1038
    var self = this;                                                                                                 // 1039
    return self._stream.status.apply(self._stream, arguments);                                                       // 1040
  },                                                                                                                 // 1041
                                                                                                                     // 1042
  /**                                                                                                                // 1043
   * @summary Force an immediate reconnection attempt if the client is not connected to the server.                  // 1044
                                                                                                                     // 1045
  This method does nothing if the client is already connected.                                                       // 1046
   * @locus Client                                                                                                   // 1047
   * @memberOf Meteor                                                                                                // 1048
   */                                                                                                                // 1049
  reconnect: function (/*passthrough args*/) {                                                                       // 1050
    var self = this;                                                                                                 // 1051
    return self._stream.reconnect.apply(self._stream, arguments);                                                    // 1052
  },                                                                                                                 // 1053
                                                                                                                     // 1054
  /**                                                                                                                // 1055
   * @summary Disconnect the client from the server.                                                                 // 1056
   * @locus Client                                                                                                   // 1057
   * @memberOf Meteor                                                                                                // 1058
   */                                                                                                                // 1059
  disconnect: function (/*passthrough args*/) {                                                                      // 1060
    var self = this;                                                                                                 // 1061
    return self._stream.disconnect.apply(self._stream, arguments);                                                   // 1062
  },                                                                                                                 // 1063
                                                                                                                     // 1064
  close: function () {                                                                                               // 1065
    var self = this;                                                                                                 // 1066
    return self._stream.disconnect({_permanent: true});                                                              // 1067
  },                                                                                                                 // 1068
                                                                                                                     // 1069
  ///                                                                                                                // 1070
  /// Reactive user system                                                                                           // 1071
  ///                                                                                                                // 1072
  userId: function () {                                                                                              // 1073
    var self = this;                                                                                                 // 1074
    if (self._userIdDeps)                                                                                            // 1075
      self._userIdDeps.depend();                                                                                     // 1076
    return self._userId;                                                                                             // 1077
  },                                                                                                                 // 1078
                                                                                                                     // 1079
  setUserId: function (userId) {                                                                                     // 1080
    var self = this;                                                                                                 // 1081
    // Avoid invalidating dependents if setUserId is called with current value.                                      // 1082
    if (self._userId === userId)                                                                                     // 1083
      return;                                                                                                        // 1084
    self._userId = userId;                                                                                           // 1085
    if (self._userIdDeps)                                                                                            // 1086
      self._userIdDeps.changed();                                                                                    // 1087
  },                                                                                                                 // 1088
                                                                                                                     // 1089
  // Returns true if we are in a state after reconnect of waiting for subs to be                                     // 1090
  // revived or early methods to finish their data, or we are waiting for a                                          // 1091
  // "wait" method to finish.                                                                                        // 1092
  _waitingForQuiescence: function () {                                                                               // 1093
    var self = this;                                                                                                 // 1094
    return (! _.isEmpty(self._subsBeingRevived) ||                                                                   // 1095
            ! _.isEmpty(self._methodsBlockingQuiescence));                                                           // 1096
  },                                                                                                                 // 1097
                                                                                                                     // 1098
  // Returns true if any method whose message has been sent to the server has                                        // 1099
  // not yet invoked its user callback.                                                                              // 1100
  _anyMethodsAreOutstanding: function () {                                                                           // 1101
    var self = this;                                                                                                 // 1102
    return _.any(_.pluck(self._methodInvokers, 'sentMessage'));                                                      // 1103
  },                                                                                                                 // 1104
                                                                                                                     // 1105
  _livedata_connected: function (msg) {                                                                              // 1106
    var self = this;                                                                                                 // 1107
                                                                                                                     // 1108
    if (self._version !== 'pre1' && self._heartbeatInterval !== 0) {                                                 // 1109
      self._heartbeat = new DDPCommon.Heartbeat({                                                                    // 1110
        heartbeatInterval: self._heartbeatInterval,                                                                  // 1111
        heartbeatTimeout: self._heartbeatTimeout,                                                                    // 1112
        onTimeout: function () {                                                                                     // 1113
          self._lostConnection(                                                                                      // 1114
            new DDP.ConnectionError("DDP heartbeat timed out"));                                                     // 1115
        },                                                                                                           // 1116
        sendPing: function () {                                                                                      // 1117
          self._send({msg: 'ping'});                                                                                 // 1118
        }                                                                                                            // 1119
      });                                                                                                            // 1120
      self._heartbeat.start();                                                                                       // 1121
    }                                                                                                                // 1122
                                                                                                                     // 1123
    // If this is a reconnect, we'll have to reset all stores.                                                       // 1124
    if (self._lastSessionId)                                                                                         // 1125
      self._resetStores = true;                                                                                      // 1126
                                                                                                                     // 1127
    if (typeof (msg.session) === "string") {                                                                         // 1128
      var reconnectedToPreviousSession = (self._lastSessionId === msg.session);                                      // 1129
      self._lastSessionId = msg.session;                                                                             // 1130
    }                                                                                                                // 1131
                                                                                                                     // 1132
    if (reconnectedToPreviousSession) {                                                                              // 1133
      // Successful reconnection -- pick up where we left off.  Note that right                                      // 1134
      // now, this never happens: the server never connects us to a previous                                         // 1135
      // session, because DDP doesn't provide enough data for the server to know                                     // 1136
      // what messages the client has processed. We need to improve DDP to make                                      // 1137
      // this possible, at which point we'll probably need more code here.                                           // 1138
      return;                                                                                                        // 1139
    }                                                                                                                // 1140
                                                                                                                     // 1141
    // Server doesn't have our data any more. Re-sync a new session.                                                 // 1142
                                                                                                                     // 1143
    // Forget about messages we were buffering for unknown collections. They'll                                      // 1144
    // be resent if still relevant.                                                                                  // 1145
    self._updatesForUnknownStores = {};                                                                              // 1146
                                                                                                                     // 1147
    if (self._resetStores) {                                                                                         // 1148
      // Forget about the effects of stubs. We'll be resetting all collections                                       // 1149
      // anyway.                                                                                                     // 1150
      self._documentsWrittenByStub = {};                                                                             // 1151
      self._serverDocuments = {};                                                                                    // 1152
    }                                                                                                                // 1153
                                                                                                                     // 1154
    // Clear _afterUpdateCallbacks.                                                                                  // 1155
    self._afterUpdateCallbacks = [];                                                                                 // 1156
                                                                                                                     // 1157
    // Mark all named subscriptions which are ready (ie, we already called the                                       // 1158
    // ready callback) as needing to be revived.                                                                     // 1159
    // XXX We should also block reconnect quiescence until unnamed subscriptions                                     // 1160
    //     (eg, autopublish) are done re-publishing to avoid flicker!                                                // 1161
    self._subsBeingRevived = {};                                                                                     // 1162
    _.each(self._subscriptions, function (sub, id) {                                                                 // 1163
      if (sub.ready)                                                                                                 // 1164
        self._subsBeingRevived[id] = true;                                                                           // 1165
    });                                                                                                              // 1166
                                                                                                                     // 1167
    // Arrange for "half-finished" methods to have their callbacks run, and                                          // 1168
    // track methods that were sent on this connection so that we don't                                              // 1169
    // quiesce until they are all done.                                                                              // 1170
    //                                                                                                               // 1171
    // Start by clearing _methodsBlockingQuiescence: methods sent before                                             // 1172
    // reconnect don't matter, and any "wait" methods sent on the new connection                                     // 1173
    // that we drop here will be restored by the loop below.                                                         // 1174
    self._methodsBlockingQuiescence = {};                                                                            // 1175
    if (self._resetStores) {                                                                                         // 1176
      _.each(self._methodInvokers, function (invoker) {                                                              // 1177
        if (invoker.gotResult()) {                                                                                   // 1178
          // This method already got its result, but it didn't call its callback                                     // 1179
          // because its data didn't become visible. We did not resend the                                           // 1180
          // method RPC. We'll call its callback when we get a full quiesce,                                         // 1181
          // since that's as close as we'll get to "data must be visible".                                           // 1182
          self._afterUpdateCallbacks.push(_.bind(invoker.dataVisible, invoker));                                     // 1183
        } else if (invoker.sentMessage) {                                                                            // 1184
          // This method has been sent on this connection (maybe as a resend                                         // 1185
          // from the last connection, maybe from onReconnect, maybe just very                                       // 1186
          // quickly before processing the connected message).                                                       // 1187
          //                                                                                                         // 1188
          // We don't need to do anything special to ensure its callbacks get                                        // 1189
          // called, but we'll count it as a method which is preventing                                              // 1190
          // reconnect quiescence. (eg, it might be a login method that was run                                      // 1191
          // from onReconnect, and we don't want to see flicker by seeing a                                          // 1192
          // logged-out state.)                                                                                      // 1193
          self._methodsBlockingQuiescence[invoker.methodId] = true;                                                  // 1194
        }                                                                                                            // 1195
      });                                                                                                            // 1196
    }                                                                                                                // 1197
                                                                                                                     // 1198
    self._messagesBufferedUntilQuiescence = [];                                                                      // 1199
                                                                                                                     // 1200
    // If we're not waiting on any methods or subs, we can reset the stores and                                      // 1201
    // call the callbacks immediately.                                                                               // 1202
    if (!self._waitingForQuiescence()) {                                                                             // 1203
      if (self._resetStores) {                                                                                       // 1204
        _.each(self._stores, function (s) {                                                                          // 1205
          s.beginUpdate(0, true);                                                                                    // 1206
          s.endUpdate();                                                                                             // 1207
        });                                                                                                          // 1208
        self._resetStores = false;                                                                                   // 1209
      }                                                                                                              // 1210
      self._runAfterUpdateCallbacks();                                                                               // 1211
    }                                                                                                                // 1212
  },                                                                                                                 // 1213
                                                                                                                     // 1214
                                                                                                                     // 1215
  _processOneDataMessage: function (msg, updates) {                                                                  // 1216
    var self = this;                                                                                                 // 1217
    // Using underscore here so as not to need to capitalize.                                                        // 1218
    self['_process_' + msg.msg](msg, updates);                                                                       // 1219
  },                                                                                                                 // 1220
                                                                                                                     // 1221
                                                                                                                     // 1222
  _livedata_data: function (msg) {                                                                                   // 1223
    var self = this;                                                                                                 // 1224
                                                                                                                     // 1225
    // collection name -> array of messages                                                                          // 1226
    var updates = {};                                                                                                // 1227
                                                                                                                     // 1228
    if (self._waitingForQuiescence()) {                                                                              // 1229
      self._messagesBufferedUntilQuiescence.push(msg);                                                               // 1230
                                                                                                                     // 1231
      if (msg.msg === "nosub")                                                                                       // 1232
        delete self._subsBeingRevived[msg.id];                                                                       // 1233
                                                                                                                     // 1234
      _.each(msg.subs || [], function (subId) {                                                                      // 1235
        delete self._subsBeingRevived[subId];                                                                        // 1236
      });                                                                                                            // 1237
      _.each(msg.methods || [], function (methodId) {                                                                // 1238
        delete self._methodsBlockingQuiescence[methodId];                                                            // 1239
      });                                                                                                            // 1240
                                                                                                                     // 1241
      if (self._waitingForQuiescence())                                                                              // 1242
        return;                                                                                                      // 1243
                                                                                                                     // 1244
      // No methods or subs are blocking quiescence!                                                                 // 1245
      // We'll now process and all of our buffered messages, reset all stores,                                       // 1246
      // and apply them all at once.                                                                                 // 1247
      _.each(self._messagesBufferedUntilQuiescence, function (bufferedMsg) {                                         // 1248
        self._processOneDataMessage(bufferedMsg, updates);                                                           // 1249
      });                                                                                                            // 1250
      self._messagesBufferedUntilQuiescence = [];                                                                    // 1251
    } else {                                                                                                         // 1252
      self._processOneDataMessage(msg, updates);                                                                     // 1253
    }                                                                                                                // 1254
                                                                                                                     // 1255
    if (self._resetStores || !_.isEmpty(updates)) {                                                                  // 1256
      // Begin a transactional update of each store.                                                                 // 1257
      _.each(self._stores, function (s, storeName) {                                                                 // 1258
        s.beginUpdate(_.has(updates, storeName) ? updates[storeName].length : 0,                                     // 1259
                      self._resetStores);                                                                            // 1260
      });                                                                                                            // 1261
      self._resetStores = false;                                                                                     // 1262
                                                                                                                     // 1263
      _.each(updates, function (updateMessages, storeName) {                                                         // 1264
        var store = self._stores[storeName];                                                                         // 1265
        if (store) {                                                                                                 // 1266
          _.each(updateMessages, function (updateMessage) {                                                          // 1267
            store.update(updateMessage);                                                                             // 1268
          });                                                                                                        // 1269
        } else {                                                                                                     // 1270
          // Nobody's listening for this data. Queue it up until                                                     // 1271
          // someone wants it.                                                                                       // 1272
          // XXX memory use will grow without bound if you forget to                                                 // 1273
          // create a collection or just don't care about it... going                                                // 1274
          // to have to do something about that.                                                                     // 1275
          if (!_.has(self._updatesForUnknownStores, storeName))                                                      // 1276
            self._updatesForUnknownStores[storeName] = [];                                                           // 1277
          Array.prototype.push.apply(self._updatesForUnknownStores[storeName],                                       // 1278
                                     updateMessages);                                                                // 1279
        }                                                                                                            // 1280
      });                                                                                                            // 1281
                                                                                                                     // 1282
      // End update transaction.                                                                                     // 1283
      _.each(self._stores, function (s) { s.endUpdate(); });                                                         // 1284
    }                                                                                                                // 1285
                                                                                                                     // 1286
    self._runAfterUpdateCallbacks();                                                                                 // 1287
  },                                                                                                                 // 1288
                                                                                                                     // 1289
  // Call any callbacks deferred with _runWhenAllServerDocsAreFlushed whose                                          // 1290
  // relevant docs have been flushed, as well as dataVisible callbacks at                                            // 1291
  // reconnect-quiescence time.                                                                                      // 1292
  _runAfterUpdateCallbacks: function () {                                                                            // 1293
    var self = this;                                                                                                 // 1294
    var callbacks = self._afterUpdateCallbacks;                                                                      // 1295
    self._afterUpdateCallbacks = [];                                                                                 // 1296
    _.each(callbacks, function (c) {                                                                                 // 1297
      c();                                                                                                           // 1298
    });                                                                                                              // 1299
  },                                                                                                                 // 1300
                                                                                                                     // 1301
  _pushUpdate: function (updates, collection, msg) {                                                                 // 1302
    var self = this;                                                                                                 // 1303
    if (!_.has(updates, collection)) {                                                                               // 1304
      updates[collection] = [];                                                                                      // 1305
    }                                                                                                                // 1306
    updates[collection].push(msg);                                                                                   // 1307
  },                                                                                                                 // 1308
                                                                                                                     // 1309
  _getServerDoc: function (collection, id) {                                                                         // 1310
    var self = this;                                                                                                 // 1311
    if (!_.has(self._serverDocuments, collection))                                                                   // 1312
      return null;                                                                                                   // 1313
    var serverDocsForCollection = self._serverDocuments[collection];                                                 // 1314
    return serverDocsForCollection.get(id) || null;                                                                  // 1315
  },                                                                                                                 // 1316
                                                                                                                     // 1317
  _process_added: function (msg, updates) {                                                                          // 1318
    var self = this;                                                                                                 // 1319
    var id = MongoID.idParse(msg.id);                                                                                // 1320
    var serverDoc = self._getServerDoc(msg.collection, id);                                                          // 1321
    if (serverDoc) {                                                                                                 // 1322
      // Some outstanding stub wrote here.                                                                           // 1323
      var isExisting = (serverDoc.document !== undefined);                                                           // 1324
                                                                                                                     // 1325
      serverDoc.document = msg.fields || {};                                                                         // 1326
      serverDoc.document._id = id;                                                                                   // 1327
                                                                                                                     // 1328
      if (self._resetStores) {                                                                                       // 1329
        // During reconnect the server is sending adds for existing ids.                                             // 1330
        // Always push an update so that document stays in the store after                                           // 1331
        // reset. Use current version of the document for this update, so                                            // 1332
        // that stub-written values are preserved.                                                                   // 1333
        var currentDoc = self._stores[msg.collection].getDoc(msg.id);                                                // 1334
        if (currentDoc !== undefined)                                                                                // 1335
          msg.fields = currentDoc;                                                                                   // 1336
                                                                                                                     // 1337
        self._pushUpdate(updates, msg.collection, msg);                                                              // 1338
      } else if (isExisting) {                                                                                       // 1339
        throw new Error("Server sent add for existing id: " + msg.id);                                               // 1340
      }                                                                                                              // 1341
    } else {                                                                                                         // 1342
      self._pushUpdate(updates, msg.collection, msg);                                                                // 1343
    }                                                                                                                // 1344
  },                                                                                                                 // 1345
                                                                                                                     // 1346
  _process_changed: function (msg, updates) {                                                                        // 1347
    var self = this;                                                                                                 // 1348
    var serverDoc = self._getServerDoc(                                                                              // 1349
      msg.collection, MongoID.idParse(msg.id));                                                                      // 1350
    if (serverDoc) {                                                                                                 // 1351
      if (serverDoc.document === undefined)                                                                          // 1352
        throw new Error("Server sent changed for nonexisting id: " + msg.id);                                        // 1353
      DiffSequence.applyChanges(serverDoc.document, msg.fields);                                                     // 1354
    } else {                                                                                                         // 1355
      self._pushUpdate(updates, msg.collection, msg);                                                                // 1356
    }                                                                                                                // 1357
  },                                                                                                                 // 1358
                                                                                                                     // 1359
  _process_removed: function (msg, updates) {                                                                        // 1360
    var self = this;                                                                                                 // 1361
    var serverDoc = self._getServerDoc(                                                                              // 1362
      msg.collection, MongoID.idParse(msg.id));                                                                      // 1363
    if (serverDoc) {                                                                                                 // 1364
      // Some outstanding stub wrote here.                                                                           // 1365
      if (serverDoc.document === undefined)                                                                          // 1366
        throw new Error("Server sent removed for nonexisting id:" + msg.id);                                         // 1367
      serverDoc.document = undefined;                                                                                // 1368
    } else {                                                                                                         // 1369
      self._pushUpdate(updates, msg.collection, {                                                                    // 1370
        msg: 'removed',                                                                                              // 1371
        collection: msg.collection,                                                                                  // 1372
        id: msg.id                                                                                                   // 1373
      });                                                                                                            // 1374
    }                                                                                                                // 1375
  },                                                                                                                 // 1376
                                                                                                                     // 1377
  _process_updated: function (msg, updates) {                                                                        // 1378
    var self = this;                                                                                                 // 1379
    // Process "method done" messages.                                                                               // 1380
    _.each(msg.methods, function (methodId) {                                                                        // 1381
      _.each(self._documentsWrittenByStub[methodId], function (written) {                                            // 1382
        var serverDoc = self._getServerDoc(written.collection, written.id);                                          // 1383
        if (!serverDoc)                                                                                              // 1384
          throw new Error("Lost serverDoc for " + JSON.stringify(written));                                          // 1385
        if (!serverDoc.writtenByStubs[methodId])                                                                     // 1386
          throw new Error("Doc " + JSON.stringify(written) +                                                         // 1387
                          " not written by  method " + methodId);                                                    // 1388
        delete serverDoc.writtenByStubs[methodId];                                                                   // 1389
        if (_.isEmpty(serverDoc.writtenByStubs)) {                                                                   // 1390
          // All methods whose stubs wrote this method have completed! We can                                        // 1391
          // now copy the saved document to the database (reverting the stub's                                       // 1392
          // change if the server did not write to this object, or applying the                                      // 1393
          // server's writes if it did).                                                                             // 1394
                                                                                                                     // 1395
          // This is a fake ddp 'replace' message.  It's just for talking                                            // 1396
          // between livedata connections and minimongo.  (We have to stringify                                      // 1397
          // the ID because it's supposed to look like a wire message.)                                              // 1398
          self._pushUpdate(updates, written.collection, {                                                            // 1399
            msg: 'replace',                                                                                          // 1400
            id: MongoID.idStringify(written.id),                                                                     // 1401
            replace: serverDoc.document                                                                              // 1402
          });                                                                                                        // 1403
          // Call all flush callbacks.                                                                               // 1404
          _.each(serverDoc.flushCallbacks, function (c) {                                                            // 1405
            c();                                                                                                     // 1406
          });                                                                                                        // 1407
                                                                                                                     // 1408
          // Delete this completed serverDocument. Don't bother to GC empty                                          // 1409
          // IdMaps inside self._serverDocuments, since there probably aren't                                        // 1410
          // many collections and they'll be written repeatedly.                                                     // 1411
          self._serverDocuments[written.collection].remove(written.id);                                              // 1412
        }                                                                                                            // 1413
      });                                                                                                            // 1414
      delete self._documentsWrittenByStub[methodId];                                                                 // 1415
                                                                                                                     // 1416
      // We want to call the data-written callback, but we can't do so until all                                     // 1417
      // currently buffered messages are flushed.                                                                    // 1418
      var callbackInvoker = self._methodInvokers[methodId];                                                          // 1419
      if (!callbackInvoker)                                                                                          // 1420
        throw new Error("No callback invoker for method " + methodId);                                               // 1421
      self._runWhenAllServerDocsAreFlushed(                                                                          // 1422
        _.bind(callbackInvoker.dataVisible, callbackInvoker));                                                       // 1423
    });                                                                                                              // 1424
  },                                                                                                                 // 1425
                                                                                                                     // 1426
  _process_ready: function (msg, updates) {                                                                          // 1427
    var self = this;                                                                                                 // 1428
    // Process "sub ready" messages. "sub ready" messages don't take effect                                          // 1429
    // until all current server documents have been flushed to the local                                             // 1430
    // database. We can use a write fence to implement this.                                                         // 1431
    _.each(msg.subs, function (subId) {                                                                              // 1432
      self._runWhenAllServerDocsAreFlushed(function () {                                                             // 1433
        var subRecord = self._subscriptions[subId];                                                                  // 1434
        // Did we already unsubscribe?                                                                               // 1435
        if (!subRecord)                                                                                              // 1436
          return;                                                                                                    // 1437
        // Did we already receive a ready message? (Oops!)                                                           // 1438
        if (subRecord.ready)                                                                                         // 1439
          return;                                                                                                    // 1440
        subRecord.ready = true;                                                                                      // 1441
        subRecord.readyCallback && subRecord.readyCallback();                                                        // 1442
        subRecord.readyDeps.changed();                                                                               // 1443
      });                                                                                                            // 1444
    });                                                                                                              // 1445
  },                                                                                                                 // 1446
                                                                                                                     // 1447
  // Ensures that "f" will be called after all documents currently in                                                // 1448
  // _serverDocuments have been written to the local cache. f will not be called                                     // 1449
  // if the connection is lost before then!                                                                          // 1450
  _runWhenAllServerDocsAreFlushed: function (f) {                                                                    // 1451
    var self = this;                                                                                                 // 1452
    var runFAfterUpdates = function () {                                                                             // 1453
      self._afterUpdateCallbacks.push(f);                                                                            // 1454
    };                                                                                                               // 1455
    var unflushedServerDocCount = 0;                                                                                 // 1456
    var onServerDocFlush = function () {                                                                             // 1457
      --unflushedServerDocCount;                                                                                     // 1458
      if (unflushedServerDocCount === 0) {                                                                           // 1459
        // This was the last doc to flush! Arrange to run f after the updates                                        // 1460
        // have been applied.                                                                                        // 1461
        runFAfterUpdates();                                                                                          // 1462
      }                                                                                                              // 1463
    };                                                                                                               // 1464
    _.each(self._serverDocuments, function (collectionDocs) {                                                        // 1465
      collectionDocs.forEach(function (serverDoc) {                                                                  // 1466
        var writtenByStubForAMethodWithSentMessage = _.any(                                                          // 1467
          serverDoc.writtenByStubs, function (dummy, methodId) {                                                     // 1468
            var invoker = self._methodInvokers[methodId];                                                            // 1469
            return invoker && invoker.sentMessage;                                                                   // 1470
          });                                                                                                        // 1471
        if (writtenByStubForAMethodWithSentMessage) {                                                                // 1472
          ++unflushedServerDocCount;                                                                                 // 1473
          serverDoc.flushCallbacks.push(onServerDocFlush);                                                           // 1474
        }                                                                                                            // 1475
      });                                                                                                            // 1476
    });                                                                                                              // 1477
    if (unflushedServerDocCount === 0) {                                                                             // 1478
      // There aren't any buffered docs --- we can call f as soon as the current                                     // 1479
      // round of updates is applied!                                                                                // 1480
      runFAfterUpdates();                                                                                            // 1481
    }                                                                                                                // 1482
  },                                                                                                                 // 1483
                                                                                                                     // 1484
  _livedata_nosub: function (msg) {                                                                                  // 1485
    var self = this;                                                                                                 // 1486
                                                                                                                     // 1487
    // First pass it through _livedata_data, which only uses it to help get                                          // 1488
    // towards quiescence.                                                                                           // 1489
    self._livedata_data(msg);                                                                                        // 1490
                                                                                                                     // 1491
    // Do the rest of our processing immediately, with no                                                            // 1492
    // buffering-until-quiescence.                                                                                   // 1493
                                                                                                                     // 1494
    // we weren't subbed anyway, or we initiated the unsub.                                                          // 1495
    if (!_.has(self._subscriptions, msg.id))                                                                         // 1496
      return;                                                                                                        // 1497
                                                                                                                     // 1498
    // XXX COMPAT WITH 1.0.3.1 #errorCallback                                                                        // 1499
    var errorCallback = self._subscriptions[msg.id].errorCallback;                                                   // 1500
    var stopCallback = self._subscriptions[msg.id].stopCallback;                                                     // 1501
                                                                                                                     // 1502
    self._subscriptions[msg.id].remove();                                                                            // 1503
                                                                                                                     // 1504
    var meteorErrorFromMsg = function (msgArg) {                                                                     // 1505
      return msgArg && msgArg.error && new Meteor.Error(                                                             // 1506
        msgArg.error.error, msgArg.error.reason, msgArg.error.details);                                              // 1507
    }                                                                                                                // 1508
                                                                                                                     // 1509
    // XXX COMPAT WITH 1.0.3.1 #errorCallback                                                                        // 1510
    if (errorCallback && msg.error) {                                                                                // 1511
      errorCallback(meteorErrorFromMsg(msg));                                                                        // 1512
    }                                                                                                                // 1513
                                                                                                                     // 1514
    if (stopCallback) {                                                                                              // 1515
      stopCallback(meteorErrorFromMsg(msg));                                                                         // 1516
    }                                                                                                                // 1517
  },                                                                                                                 // 1518
                                                                                                                     // 1519
  _process_nosub: function () {                                                                                      // 1520
    // This is called as part of the "buffer until quiescence" process, but                                          // 1521
    // nosub's effect is always immediate. It only goes in the buffer at all                                         // 1522
    // because it's possible for a nosub to be the thing that triggers                                               // 1523
    // quiescence, if we were waiting for a sub to be revived and it dies                                            // 1524
    // instead.                                                                                                      // 1525
  },                                                                                                                 // 1526
                                                                                                                     // 1527
  _livedata_result: function (msg) {                                                                                 // 1528
    // id, result or error. error has error (code), reason, details                                                  // 1529
                                                                                                                     // 1530
    var self = this;                                                                                                 // 1531
                                                                                                                     // 1532
    // find the outstanding request                                                                                  // 1533
    // should be O(1) in nearly all realistic use cases                                                              // 1534
    if (_.isEmpty(self._outstandingMethodBlocks)) {                                                                  // 1535
      Meteor._debug("Received method result but no methods outstanding");                                            // 1536
      return;                                                                                                        // 1537
    }                                                                                                                // 1538
    var currentMethodBlock = self._outstandingMethodBlocks[0].methods;                                               // 1539
    var m;                                                                                                           // 1540
    for (var i = 0; i < currentMethodBlock.length; i++) {                                                            // 1541
      m = currentMethodBlock[i];                                                                                     // 1542
      if (m.methodId === msg.id)                                                                                     // 1543
        break;                                                                                                       // 1544
    }                                                                                                                // 1545
                                                                                                                     // 1546
    if (!m) {                                                                                                        // 1547
      Meteor._debug("Can't match method response to original method call", msg);                                     // 1548
      return;                                                                                                        // 1549
    }                                                                                                                // 1550
                                                                                                                     // 1551
    // Remove from current method block. This may leave the block empty, but we                                      // 1552
    // don't move on to the next block until the callback has been delivered, in                                     // 1553
    // _outstandingMethodFinished.                                                                                   // 1554
    currentMethodBlock.splice(i, 1);                                                                                 // 1555
                                                                                                                     // 1556
    if (_.has(msg, 'error')) {                                                                                       // 1557
      m.receiveResult(new Meteor.Error(                                                                              // 1558
        msg.error.error, msg.error.reason,                                                                           // 1559
        msg.error.details));                                                                                         // 1560
    } else {                                                                                                         // 1561
      // msg.result may be undefined if the method didn't return a                                                   // 1562
      // value                                                                                                       // 1563
      m.receiveResult(undefined, msg.result);                                                                        // 1564
    }                                                                                                                // 1565
  },                                                                                                                 // 1566
                                                                                                                     // 1567
  // Called by MethodInvoker after a method's callback is invoked.  If this was                                      // 1568
  // the last outstanding method in the current block, runs the next block. If                                       // 1569
  // there are no more methods, consider accepting a hot code push.                                                  // 1570
  _outstandingMethodFinished: function () {                                                                          // 1571
    var self = this;                                                                                                 // 1572
    if (self._anyMethodsAreOutstanding())                                                                            // 1573
      return;                                                                                                        // 1574
                                                                                                                     // 1575
    // No methods are outstanding. This should mean that the first block of                                          // 1576
    // methods is empty. (Or it might not exist, if this was a method that                                           // 1577
    // half-finished before disconnect/reconnect.)                                                                   // 1578
    if (! _.isEmpty(self._outstandingMethodBlocks)) {                                                                // 1579
      var firstBlock = self._outstandingMethodBlocks.shift();                                                        // 1580
      if (! _.isEmpty(firstBlock.methods))                                                                           // 1581
        throw new Error("No methods outstanding but nonempty block: " +                                              // 1582
                        JSON.stringify(firstBlock));                                                                 // 1583
                                                                                                                     // 1584
      // Send the outstanding methods now in the first block.                                                        // 1585
      if (!_.isEmpty(self._outstandingMethodBlocks))                                                                 // 1586
        self._sendOutstandingMethods();                                                                              // 1587
    }                                                                                                                // 1588
                                                                                                                     // 1589
    // Maybe accept a hot code push.                                                                                 // 1590
    self._maybeMigrate();                                                                                            // 1591
  },                                                                                                                 // 1592
                                                                                                                     // 1593
  // Sends messages for all the methods in the first block in                                                        // 1594
  // _outstandingMethodBlocks.                                                                                       // 1595
  _sendOutstandingMethods: function() {                                                                              // 1596
    var self = this;                                                                                                 // 1597
    if (_.isEmpty(self._outstandingMethodBlocks))                                                                    // 1598
      return;                                                                                                        // 1599
    _.each(self._outstandingMethodBlocks[0].methods, function (m) {                                                  // 1600
      m.sendMessage();                                                                                               // 1601
    });                                                                                                              // 1602
  },                                                                                                                 // 1603
                                                                                                                     // 1604
  _livedata_error: function (msg) {                                                                                  // 1605
    Meteor._debug("Received error from server: ", msg.reason);                                                       // 1606
    if (msg.offendingMessage)                                                                                        // 1607
      Meteor._debug("For: ", msg.offendingMessage);                                                                  // 1608
  },                                                                                                                 // 1609
                                                                                                                     // 1610
  _callOnReconnectAndSendAppropriateOutstandingMethods: function() {                                                 // 1611
    var self = this;                                                                                                 // 1612
    var oldOutstandingMethodBlocks = self._outstandingMethodBlocks;                                                  // 1613
    self._outstandingMethodBlocks = [];                                                                              // 1614
                                                                                                                     // 1615
    self.onReconnect();                                                                                              // 1616
                                                                                                                     // 1617
    if (_.isEmpty(oldOutstandingMethodBlocks))                                                                       // 1618
      return;                                                                                                        // 1619
                                                                                                                     // 1620
    // We have at least one block worth of old outstanding methods to try                                            // 1621
    // again. First: did onReconnect actually send anything? If not, we just                                         // 1622
    // restore all outstanding methods and run the first block.                                                      // 1623
    if (_.isEmpty(self._outstandingMethodBlocks)) {                                                                  // 1624
      self._outstandingMethodBlocks = oldOutstandingMethodBlocks;                                                    // 1625
      self._sendOutstandingMethods();                                                                                // 1626
      return;                                                                                                        // 1627
    }                                                                                                                // 1628
                                                                                                                     // 1629
    // OK, there are blocks on both sides. Special case: merge the last block of                                     // 1630
    // the reconnect methods with the first block of the original methods, if                                        // 1631
    // neither of them are "wait" blocks.                                                                            // 1632
    if (!_.last(self._outstandingMethodBlocks).wait &&                                                               // 1633
        !oldOutstandingMethodBlocks[0].wait) {                                                                       // 1634
      _.each(oldOutstandingMethodBlocks[0].methods, function (m) {                                                   // 1635
        _.last(self._outstandingMethodBlocks).methods.push(m);                                                       // 1636
                                                                                                                     // 1637
        // If this "last block" is also the first block, send the message.                                           // 1638
        if (self._outstandingMethodBlocks.length === 1)                                                              // 1639
          m.sendMessage();                                                                                           // 1640
      });                                                                                                            // 1641
                                                                                                                     // 1642
      oldOutstandingMethodBlocks.shift();                                                                            // 1643
    }                                                                                                                // 1644
                                                                                                                     // 1645
    // Now add the rest of the original blocks on.                                                                   // 1646
    _.each(oldOutstandingMethodBlocks, function (block) {                                                            // 1647
      self._outstandingMethodBlocks.push(block);                                                                     // 1648
    });                                                                                                              // 1649
  },                                                                                                                 // 1650
                                                                                                                     // 1651
  // We can accept a hot code push if there are no methods in flight.                                                // 1652
  _readyToMigrate: function() {                                                                                      // 1653
    var self = this;                                                                                                 // 1654
    return _.isEmpty(self._methodInvokers);                                                                          // 1655
  },                                                                                                                 // 1656
                                                                                                                     // 1657
  // If we were blocking a migration, see if it's now possible to continue.                                          // 1658
  // Call whenever the set of outstanding/blocked methods shrinks.                                                   // 1659
  _maybeMigrate: function () {                                                                                       // 1660
    var self = this;                                                                                                 // 1661
    if (self._retryMigrate && self._readyToMigrate()) {                                                              // 1662
      self._retryMigrate();                                                                                          // 1663
      self._retryMigrate = null;                                                                                     // 1664
    }                                                                                                                // 1665
  }                                                                                                                  // 1666
});                                                                                                                  // 1667
                                                                                                                     // 1668
LivedataTest.Connection = Connection;                                                                                // 1669
                                                                                                                     // 1670
// @param url {String} URL to Meteor app,                                                                            // 1671
//     e.g.:                                                                                                         // 1672
//     "subdomain.meteor.com",                                                                                       // 1673
//     "http://subdomain.meteor.com",                                                                                // 1674
//     "/",                                                                                                          // 1675
//     "ddp+sockjs://ddp--****-foo.meteor.com/sockjs"                                                                // 1676
                                                                                                                     // 1677
/**                                                                                                                  // 1678
 * @summary Connect to the server of a different Meteor application to subscribe to its document sets and invoke its remote methods.
 * @locus Anywhere                                                                                                   // 1680
 * @param {String} url The URL of another Meteor application.                                                        // 1681
 */                                                                                                                  // 1682
DDP.connect = function (url, options) {                                                                              // 1683
  var ret = new Connection(url, options);                                                                            // 1684
  allConnections.push(ret); // hack. see below.                                                                      // 1685
  return ret;                                                                                                        // 1686
};                                                                                                                   // 1687
                                                                                                                     // 1688
// Hack for `spiderable` package: a way to see if the page is done                                                   // 1689
// loading all the data it needs.                                                                                    // 1690
//                                                                                                                   // 1691
allConnections = [];                                                                                                 // 1692
DDP._allSubscriptionsReady = function () {                                                                           // 1693
  return _.all(allConnections, function (conn) {                                                                     // 1694
    return _.all(conn._subscriptions, function (sub) {                                                               // 1695
      return sub.ready;                                                                                              // 1696
    });                                                                                                              // 1697
  });                                                                                                                // 1698
};                                                                                                                   // 1699
                                                                                                                     // 1700
